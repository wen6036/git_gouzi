...
class Studioinfo extends Controller
{
	...
	public function get_info(){
		...
		if($type=='netProfit'){
			$url = "http://49.235.36.29/accountPerformance/".$futures_company."_" .$futures_account."/day.txt";
			$enurl = iconv('utf-8','gbk',$url);
			$info = @file_get_contents($enurl);
			if(!$info){
				$info = file_get_contents($path."\\day.txt");	
			...
			// $url = "http://49.235.36.29/accountPerformance/".$futures_company."_" .$futures_account."/day.txt";
			$url = "http://49.235.36.29/accountPerformance/".$futures_company."_" .$futures_account."/deposit.txt";
			$url2 = "http://49.235.36.29/accountPerformance/".$futures_company."_" .$futures_account."/withdraw.txt";
			$enurl = iconv('utf-8','gbk',$url);
			$enurl2 = iconv('utf-8','gbk',$url2);
			$info = @file_get_contents($enurl);
			$info2 = @file_get_contents($enurl2);
			if(!$info){
				$info = file_get_contents($path."\\deposit.txt");	
				$info2 = file_get_contents($path."\\withdraw.txt");	
			}
	        file_put_contents($path."\\deposit.txt",$info);
	        file_put_contents($path."\\withdraw.txt",$info2);
	        $arr = parse_ini_string($info);
	        $arr2 = parse_ini_string($info2);

	        foreach ($arr as $key => $value) {
	        	$array[] = ["$key",$value -  $arr2[$key]];
	        }
		...
		//单独的状态
		$url = "http://49.235.36.29/accountPerformance/".$futures_company."_" .$futures_account."/".$type.".txt";
		$enurl = iconv('utf-8','gbk',$url);
		$info = @file_get_contents($enurl);
		if(!$info){
	...
	public function get_info2(){
		...
		try{
			...
			if($type=='netProfit'){//每日净利
				...
				$url = "http://49.235.36.29/accountPerformance/".$futures_company."_" .$futures_account."/dayinDealDays.txt";
				//截止每日日内交易天数
				$enurl = iconv('utf-8','gbk',$url);
				$info = @file_get_contents($enurl);
	            ...
	            $dayinDealDays = end($p);
	            $url = "http://49.235.36.29/accountPerformance/".$futures_company."_" .$futures_account."/dealDays.txt";
				$enurl = iconv('utf-8','gbk',$url);
				$info = @file_get_contents($enurl);
				...
				$url = "http://49.235.36.29/accountPerformance/".$futures_company."_" .$futures_account."/monthProfit.txt";
				$enurl = iconv('utf-8','gbk',$url);
				$info = @file_get_contents($enurl);
	        ...
	        if($type=='prdID_winRate'){
	        	...
	        	arsort($arr);
	        	$html = '';
	        	$html .= '<div class="js-content"><table class="table table-striped" style="width: 80%;margin: 0 auto;"><tr><th>品种</th><th>手续费(元)</th></tr><tbody id="content" class="table-b">';
	...
	public function timehold(){
		...
		$param = $this->request->param();
		$id = $param['id'];
		$studio_info = Db::table('tz_studio')->where("id=$id")->field("futures_account,futures_company,BrokerID,uid")->find();
		// $where['brokerID'] = $studio_info['BrokerID'];
		$where['futures_company'] = $studio_info['futures_company'];
		$where['userID'] = $studio_info['futures_account'];
		// ROM_UNIXTIME( datex,  '%Y%m%d' ) =20120711
