...
class WechatOauth extends Common
{
    ...
    public function getjcUserInfo($access_token, $openid)
    {
        $result = Tools::httpGet("https://api.weixin.qq.com/cgi-bin/user/info?access_token={$access_token}&openid={$openid}&lang=zh_CN");
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
