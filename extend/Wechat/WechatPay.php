...
class WechatPay
{
     ...
     * 支付通知验证处理
     * @return bool|array
     * @@param array $notice null|array
     */
    public function getNotify($notice=null)
    {
        $notifyInfo = empty($notice)?(array)simplexml_load_string(file_get_contents("php://input"), 'SimpleXMLElement', LIBXML_NOCDATA):$notice;
        if (empty($notifyInfo)) {
     ...
     * @return bool|string
     */
    public function getPrepayId($openid, $body, $out_trade_no, $total_fee, $notify_url,$attach=null, $trade_type = "JSAPI", $goods_tag = null, $fee_type = 'CNY')
    {
        $postdata = array(
            "body"             => $body,
            "out_trade_no"     => $out_trade_no,
        ...
        empty($attach) || $postdata['attach']=json_encode($attach);
     ...
     * @param string $goods_tag 商品标记，代金券或立减优惠功能的参数
     * @param string $fee_type 交易币种
     * @param array $scene_info array("type"=>"Wap","wap_url"=>"http://cash.weijing.com","wap_name"=>"支付测试");
     * @return bool|string
     */
    public function H5PrepayId($openid, $body, $out_trade_no, $total_fee, $notify_url,array $scene_info,$attach=null,$trade_type = "MWEB", $goods_tag = null, $fee_type = 'CNY')
    {
        $postdata = array(
            "body"             => $body,
            "out_trade_no"     => $out_trade_no,
        ...
        empty($goods_tag) || $postdata['goods_tag'] = $goods_tag;
        empty($openid) || $postdata['openid'] = $openid;
        empty($scene_info) || $postdata['scene_info']=json_encode($scene_info);
        empty($attach) || $postdata['attach']=json_encode($attach);
        $result = $this->getArrayResult($postdata, self::MCH_BASE_URL . '/pay/unifiedorder');
        if (false === $this->_parseResult($result)) {
            return false;
        }
        return in_array($trade_type, array('MWEB')) ?  $result['mweb_url']: $result['prepay_id'];
    }
    ...
    public function getQrUrl($openid, $body, $out_trade_no, $total_fee, $notify_url, $attach="",$goods_tag = null, $fee_type = 'CNY')
    {
        $postdata = array(
            "body"             => $body,
            "out_trade_no"     => $out_trade_no,
            ...
            "total_fee"        => $total_fee,
            "notify_url"       => $notify_url,
            'attach'           => $attach ? json_encode($attach) : '',
            "trade_type"       => 'NATIVE',
            "spbill_create_ip" => Tools::getAddress()
        );
        empty($goods_tag) || $postdata['goods_tag'] = $goods_tag;
